...
void OdomEstimationClass::updatePointsToMap(const pcl::PointCloud<pcl::PointXYZI>::Ptr &edge_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &surf_in)
{
    ...
    if (laserCloudCornerMap->points.size() > 10 && laserCloudSurfMap->points.size() > 50)
    {
        ...
        for (int iterCount = 0; iterCount < optimization_count; iterCount++)
        {
            typedef g2o::BlockSolver<g2o::BlockSolverTraits<6, 1>> BlockSolverType; //   每个误差项优化变量的维度为6,误差值维度为1
            ...
            //  增加顶点
            TESTG2O::FLOAMVertex *v = new TESTG2O::FLOAMVertex();
            v->setEstimate(SE3_Rt); //  需要传入初始值
            v->setId(0);
            opt.addVertex(v);
...
#ifndef USE_G2O
void OdomEstimationClass::addEdgeCostFactor(const pcl::PointCloud<pcl::PointXYZI>::Ptr &pc_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &map_in,
...
void OdomEstimationClass::addSurfCostFactor(const pcl::PointCloud<pcl::PointXYZI>::Ptr &pc_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &map_in,
                                            ceres::Problem &problem, ceres::LossFunction *loss_function)
{
    int surf_num = 0;
    for (int i = 0; i < (int)pc_in->points.size(); i++)
...
#else
void OdomEstimationClass::addEdgeCostFactor(const pcl::PointCloud<pcl::PointXYZI>::Ptr &pc_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr &map_in,
                                            g2o::SparseOptimizer &opt, TESTG2O::FLOAMVertex *v)
{
    int corner_num = 0;
    for (int i = 0; i < (int)pc_in->points.size(); i++)
    {
        ...
        if (pointSearchSqDis[4] < 1.0)
        {
            ...
            if (saes.eigenvalues()[2] > 3 * saes.eigenvalues()[1])
            {
                ...
                point_b = -0.1 * unit_direction + point_on_line;
                //    加入边
                TESTG2O::FLOAMEdge *edge = new TESTG2O::FLOAMEdge(point_a, point_b, curr_point);
                edge->setId(i);
                edge->setVertex(0, v);
                                            ...
                                            g2o::SparseOptimizer &opt, TESTG2O::FLOAMVertex *v)
{
    ...
    for (int i = 0; i < (int)pc_in->points.size(); i++)
    {
        ...
        if (pointSearchSqDis[4] < 1.0)
        {
            ...
            {
                //    加入边
                TESTG2O::FLOAMSurf *edge = new TESTG2O::FLOAMSurf(curr_point, norm);
                edge->setId(i);
                edge->setVertex(0, v);
